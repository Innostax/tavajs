        ...
        const initAuth0 = async () => {
            ...
            if (isAuthenticated) {
                ...
                if (isTestAuthenticated) {
                    ...
                    // user.userRole = USER_ROLES_ENUMS.FFLAccountOwner
                }
                // dispatch(setJwtToken(token))
                // dispatch(setUser(user))
                // dispatch(fetchUserProfile({ id: user.sub, showNoError: true })).then(
                //     (response) => {
                //         if (!response.payload) {
                //             const hasUserMetadata =
                //                 user[
                //                     `${process.env.REACT_APP_AUTH0_NAMESPACE}/${USER_METADATA}`
                //                 ]
                //             hasUserMetadata && hasUserMetadata.accountType
                //                 ? dispatch(
                //                         saveUserProfile({ ...user, phone: hasUserMetadata.phoneno })
                //                   )
                //                 : dispatch(changeUser({ hasAccountType: false }))
                //         }
                //     }
                // )
            }
